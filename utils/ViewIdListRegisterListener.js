...
function ViewIdListRegisterListener (viewIdList, storage) {
  ...
  this.listenerFunction = {
    ...
    RadioGroup: function (viewId) {
      let view = ui[viewId]
      var radioGroupListener = new android.widget.RadioGroup.OnCheckedChangeListener({
        onCheckedChanged: function (group, id) {
          var count=group.getChildCount()
          for (var i = 0;i < count;i++){
              let radioButton = group.getChildAt(i);
  
              if (id == radioButton.getId()) {
                  var checked = radioButton.getText();
                  if (count == 2) {
                    initComponent2(checked);
                    _storage.put('basichookmode', checked);
                  } else {
                    initComponent(checked);
                    _storage.put('hookMode', checked);
                  }
      ...
      view.setOnCheckedChangeListener(radioGroupListener)
    }
  }
  this.restoreFunction = {
    ...
    RadioGroup: function () {
      that.viewIdList.map((viewId) => {
        ...
        for (var i = 0;i < count;i++){
            let radioButton = view.getChildAt(i);

            if (value == radioButton.getText()) {
